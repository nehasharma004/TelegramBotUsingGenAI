...
async def main_bot(message: types.Message):
    ...
    try:
        ...
        if response.chat_history:
            ...
            last_message = response.chat_history[-1].message
            # Send the message as a reply in the Telegram chat
            await bot.send_message(chat_id=message.chat.id, text=last_message)
        else:
            await message.reply("Unexpected response received. Please try again later.")
